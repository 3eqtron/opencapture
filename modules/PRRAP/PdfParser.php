  ...
  protected static function extractText($data)
  {
    ...
    foreach ($a_obj as $obj) {
      ...
      if (is_array($a_filter) && isset($a_filter[0])) {
        ...
        if (is_array($a_data) && isset($a_data[0])) {
          $data = substr($a_data[0], strlen('stream'), strlen($a_data[0]) - strlen('stream') - strlen('endstream'));
          $a_chunks[$j]['data'] = substr($data, 2, -2);
          $a_chunks[$j]['a_data'] = $data;
        }
    ...
    foreach ($a_chunks as $chunk) {
      ...
      if (isset($chunk['data'])) {
        ...
        if (trim($data) != '') {
          $text = self::extractTextElements($data);
    ...
    foreach ($lines as $line) {
      ...
      switch ($operator) {
        ...
        case 'TJ':
          ...
          $t = self::parseTextCommand(mb_substr($command, $start, $end - $start, 'UTF-8'));
          $text.= ' ' . $t;
          break;
        ...
        case 'Tj':
          $start = mb_strpos($command, '(', null, 'UTF-8') + 1;
          $end   = mb_strrpos($command, ')', null, 'UTF-8');
          $t = mb_substr($command, $start, $end - $start, 'UTF-8'); // Removes round brackets
          $text.= ' ' . $t;
          break;
